...
module.exports = class MsCall {
	 ...
	 */
	async list(service, namespace, requestData = null, endpointParameters, pageSize, { useSafeMode } = {}) {
		...
		do {
			...
			if(useSafeMode && response.statusCode >= 400) // this is needed because the loop execution should end when the responseCode is >=400
				return response;
	...
	safeList(service, namespace, requestData = null, endpointParameters, pageSize) {
		return this.list(service, namespace, requestData, endpointParameters, pageSize, { useSafeMode: true });
	}
